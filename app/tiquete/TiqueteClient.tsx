...
/** Sanitized initial payload produced server-side by getPublicTicket(). */
export interface InitialTicket {
  order: any;
  business: any;
  loyalty: any;
  approval: any;
  advertising?: any | null;
}
...
/**
 * Fruver advertising renderer (Renderizador_Publicidad, R4).
 * Renders only blocks that have content — empty/null blocks are skipped so the
 * ticket never shows an empty advertising section (R4.7). Uses only the
 * sanitized public data produced by buildAdvertisingBlocks.
 */
function AdvertisingBlocks({ advertising }: { advertising: any }) {
  if (!advertising) return null;

  const promotions: any[] = advertising.promotions || [];
  const seasonal: any[] = advertising.seasonal || [];
  const combos: any[] = advertising.combos || [];
  const socialInvite: string | null = advertising.socialInvite || null;
  const discountCode: any | null = advertising.discountCode || null;
  ...
  const fmt = (n: number) => money.format(Number(n || 0));

  return (
    <Card className="mt-4 p-4">
      <strong className="text-slate-900">🥬 Novedades y ofertas</strong>
      ...
      {promotions.length > 0 && (
        <div className="mt-3 space-y-2">
          {promotions.map((p, i) => (
            <div
              key={`promo-${i}`}
              className="rounded-lg border border-brand-200 bg-brand-50 px-3 py-2 text-sm font-semibold text-brand-800"
            >
              📣 {p.text}
            </div>
          ))}
        ...
        <div className="mt-3">
          ...
          <div className="mt-2 flex flex-wrap gap-2">
            {seasonal.map((prod, i) => (
              <span
                key={`seasonal-${i}`}
                className="inline-flex items-center gap-1 rounded-full bg-orange-100 px-3 py-1 text-sm font-semibold text-orange-800"
              >
                {prod.name} · {fmt(prod.day_price)}/{prod.unit}
              </span>
            ))}
          </div>
        </div>
      ...
      {combos.length > 0 && (
        <div className="mt-3 space-y-2">
          {combos.map((c, i) => (
            ...
            >
              🧺 Combo con {c.discount_percent}% de descuento
            </div>
          ))}
        </div>
      )}
      ...
      {discountCode && (
        <div className="mt-3 rounded-lg border border-dashed border-brand-400 bg-white px-3 py-2 text-sm text-slate-700">
          🎁 Para tu próxima compra usa el código{" "}
          <strong className="text-brand-700">{discountCode.code}</strong> y obtén{" "}
          {discountCode.discount_percent}% de descuento.
        </div>
      )}
...
/**
 * Interactive ticket island. Receives sanitized data fetched on the server
 * (`initial`) and hydrates instantly, then keeps the order fresh via polling.
 * The initial paint contains no client-side data fetch and never handles
 * service-role secrets.
 */
export default function TiqueteClient({ initial }: { initial: InitialTicket | null }) {
  const params = useSearchParams();
  const [order, setOrder] = useState<any>(initial ? normalizeOrder(initial.order) : null);
  const [business, setBusiness] = useState<any>(initial?.business || null);
  const [loyalty, setLoyalty] = useState<any>(initial?.loyalty || null);
  const [approval, setApproval] = useState<any>(approvalSafety(initial?.approval));
  const [advertising, setAdvertising] = useState<any>(initial?.advertising || null);
  const [loading, setLoading] = useState(!initial);
  const [error, setError] = useState("");
  const [paymentFeedback, setPaymentFeedback] = useState<"success" | "pending" | "failure" | null>(null);
  const [mode, setMode] = useState<Mode>(() =>
    initial ? defaultModeForPhase(phaseForStatus(normalizeOrder(initial.order).status)) : "track"
  );
  const [userOverride, setUserOverride] = useState(false);
  const hasInit = useRef(Boolean(initial));
  ...
  const fetchOrder = useCallback(async () => {
    try {
      ...
      setAdvertising(data.advertising || null);
      setError("");
        ...
        <div className="mt-6 grid grid-cols-1 gap-6 lg:grid-cols-2">
          ...
          <Card className="p-6">
            ...
            <div className="mt-6">
              ...
              <AdvertisingBlocks advertising={advertising} />
            </div>
          </Card>
